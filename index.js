...
function processSum(numberList, callback) {
  const numberListSum = numberList.reduce ((total, number) => total + number, 0);

  return callback(numberListSum);
}
...
function processContains(item, list, callback) {
  let listResult = list.includes(item)
...
function firstNamesAllCaps(runners) {
  ...
  namesCaps.push(firstNames.first_name.toUpperCase());  
  }) 
  return namesCaps;
}
